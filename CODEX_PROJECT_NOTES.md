...
- پس از انتشار سیگنال‌های ریشه و resubmit موفق sitemap در Search Console، بررسی مستقیم Google در ۲۰۲۶-۰۸-۱۴ نتیجهٔ واقعی نشان داد: برای جست‌وجوی دقیق `کافپن`، `کاف‌پن` و `Coffpen Soheil Aghayani`، صفحهٔ اصلی Coffpen اولین نتیجهٔ وبِ مرتبط بود و `about.html` نتیجهٔ بعدی. این نخستین شواهد عمومیِ قابل‌اتکا برای دیده‌شدن برند است؛ همچنان از ادعای رتبهٔ ثابت در همهٔ موقعیت‌ها پرهیز کن.
...
## وضعیت و اولویت‌های فعال — ۲۰۲۶-۰۸-۱۴

این بخش مرجع کوتاهِ کارهای انجام‌شده و محدودیت‌هایی است که نباید بعد از فشرده‌شدن context فراموش شوند:

- **P0 — تگ‌ها:** استخراج تگ از `meta name="coffpen:tags"` (و برای سازگاری از `keywords`) در `scripts/sync-posts.js` اصلاح شده است. تگ‌های عمومیِ تولیدشده مثل نام برند، «داستان کوتاه» و «دل نوشته» عمداً تگِ قابل‌فیلتر محسوب نمی‌شوند. نوشته‌های فعلی متادیتای تگِ نویسنده ندارند؛ بنابراین `uniqueTags: 0` در رجیستری فعلی طبیعی است. برای فعال‌شدن فیلتر، تگ واقعی را از پنل در فیلد تگ وارد کن، سپس `node scripts/sync-posts.js` و `npm run check` را اجرا کن.
- **P0 — صحت انتشار:** `npm run check` و workflowهای `.github/workflows/site-check.yml` و `.github/workflows/sync-posts.yml` مرجع اجباری اعتبارسنجی هستند. هر دو workflow پیش از نتیجه‌گیری `sync` و سپس check را اجرا می‌کنند تا اضافه‌شدن نوشتهٔ تازه با رجیستری قدیمی race نداشته باشد. این بررسی تعداد فایل‌های `posts/` و رجیستری، محتوای خالی، نام فایل ناامن، لینک‌های محلی، sitemap، manifest، favicon، episode تکراری و پیکربندی اعلان را کنترل می‌کند. فایل‌های تولیدی را دستی تغییر نده.
- **P1 — کش و دسترسی پنل:** `document.write` و `Date.now()` برای لود اسکریپت‌ها حذف شده و نسخهٔ ثابت `20260814-2` برای cache-busting استفاده می‌شود؛ `Date.now()`های باقی‌مانده فقط برای عمر پیش‌نویس و درخواست `no-store` هستند. قفل نام کاربری پنل فقط قفل محلی رابط است و OAuth یا احراز هویت واقعی نیست؛ توکن GitHub، service-account یا secret را هرگز داخل HTML/JS استاتیک نگذار. دسترسی واقعیِ ویرایش/ذخیره توسط GitHub انجام می‌شود.
- **P1 — اعلان‌ها:** اعلان Push تا زمان ساخت Firebase، قرار دادن config عمومی و VAPID key در `assets/js/firebase-config.js` و افزودن secretهای لازم GitHub عمداً خاموش است. این قابلیت با Firebase Spark بدون کارت بانکی قابل راه‌اندازی است، اما بدون config واقعی نباید فعال اعلام شود. آیکون اعلان از `assets/images/favicon-48.png` استفاده می‌کند.
- **P2 — لایک، کامنت و ایمیل:** GitHub Pages به‌تنهایی backend نیست. fallback کامنت فعلی فقط localStorage همان مرورگر است و بین کاربران، اعلان پنل، پاسخ و ایمیل ایجاد نمی‌کند. راه‌اندازی واقعی این بخش نیازمند سرویس backend/احراز هویت، قوانین moderation و سرویس ایمیل است؛ تا آن زمان نباید در UI یا مستندات ادعا شود که «کامنت واقعی» فعال است.
- **P2 — دارایی‌ها:** فایل‌های untracked کاربر `assets/images/paper choke.webp` و `assets/images/thumbs/paper choke.webp` متعلق به کاربرند و نباید حذف یا stage شوند. `assets/images/favicon.png` حجیم و بدون reference شناسایی شده است؛ فقط پس از تأیید مستقلِ نبودِ نیاز کاربر می‌توان آن را بهینه/حذف کرد.
- **انجام‌شدهٔ دسترس‌پذیری:** منوی همبرگری شناسهٔ ARIA، بازگرداندن focus، بستن با Escape و focus trap برای Tab دارد. این رفتار را هنگام اصلاح sidebar حفظ کن.

### دستورهای نهایی بررسی
...
اگر یکی از قابلیت‌های بیرونی (اعلان واقعی، لایک/کامنت چندکاربره یا ایمیل) درخواست شد، اول سرویس و credential موردنظر کاربر را مشخص کن؛ آن را با localStorage یا یک کلید ساختگی «حل‌شده» اعلام نکن.
