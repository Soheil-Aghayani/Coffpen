/* Auto-generated by scripts/sync-posts.js. Do not edit manually. */
window.COFFPEN_POSTS = Object.freeze([
  {
    "title": "درون باطل",
    "url": "posts/%D8%AF%D8%B1%D9%88%D9%86-%D8%A8%D8%A7%D8%B7%D9%84-%D9%82%D8%B3%D9%85%D8%AA-%DB%B1.html",
    "filename": "درون-باطل-قسمت-۱.html",
    "description": "«چی شده جان؟ آشفته به‌نظر می‌رسی.» هانس با صدایی متعجب پرسید، در حالی‌که روی مبل مورد علاقه‌ی جان نشسته بود و گام‌های پی‌درپی او را زیر لب می‌شمرد. نور خاکستری صبحگاهی از لابه‌لای…",
    "author": "سهیل آقایانی",
    "series": "درون باطل",
    "episode": "1",
    "wordCount": 609,
    "date": "2026-07-29T13:16:22.000Z",
    "empty": false
  }
]);
